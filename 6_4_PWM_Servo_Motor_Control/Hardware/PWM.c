...
void PWM_Init(void)
{
    ...
    TIM_TimeBaseInitStruct.TIM_Prescaler = 72 - 1; // Prescaler/[Set prescaler to have 10 kHz timer clock (assuming 72 MHz system clock)]
    TIM_TimeBaseInitStruct.TIM_CounterMode = TIM_CounterMode_Up; // Upcounting mode
    TIM_TimeBaseInitStruct.TIM_Period = 20000 - 1; // ARR/[Set period for 10000 counts (1 second overflow)]
    ...
    // PWM mode configuration on Output Channel 2
    ...
    TIM_OCInitStructer.TIM_Pulse = 0; // 500~2500/CCR/[Initial pulse width (0% duty cycle)]
    TIM_OC2Init(TIM2, &TIM_OCInitStructer); // Initialize PWM on Channel 2
